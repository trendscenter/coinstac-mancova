...
def local_run_mancova(args):
    ...
    for filename in glob.glob(os.path.join(state['baseDirectory'], "*")):
        if os.path.isdir(filename):
            shutil.copytree(filename, os.path.join(state['outputDirectory'], os.path.basename(filename)))
        else:
            shutil.copy(filename, os.path.join(state['outputDirectory'], os.path.basename(filename)))
    in_files = [os.path.join(state['baseDirectory'], f)
                for f in file_list]
    ut.log("Loaded files %s" % ', '.join(in_files), state)
    covariates = convert_covariates(covariate_file, state, covariate_types=covariate_type_file)
